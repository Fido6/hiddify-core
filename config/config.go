...
func setRoutingOptions(options *option.Options, opt *HiddifyOptions) {
	...
	if opt.BlockAds {
		rulesets = append(rulesets, option.RuleSet{
			...
			RemoteOptions: option.RemoteRuleSet{
				URL:            "https://fastly.jsdelivr.net/gh/SagerNet/sing-geosite@refs/heads/rule-set/geosite-category-ads-all.srs",
	...
	if opt.Region != "other" {
		...
		rulesets = append(rulesets, option.RuleSet{
			...
			Format: C.RuleSetFormatBinary,
			RemoteOptions: option.RemoteRuleSet{
				URL:            "https://fastly.jsdelivr.net/gh/SagerNet/sing-geoip@raw/refs/heads/rule-set/geoip-" + opt.Region + ".srs",
				UpdateInterval: option.Duration(5 * time.Hour * 24),
			},
		})
		rulesets = append(rulesets, option.RuleSet{
			...
			Format: C.RuleSetFormatBinary,
			RemoteOptions: option.RemoteRuleSet{
				URL:            "https://fastly.jsdelivr.net/gh/hiddify/hiddify-geo@rule-set/country/geosite-" + opt.Region + ".srs",
				UpdateInterval: option.Duration(5 * time.Hour * 24),
			},
		})
